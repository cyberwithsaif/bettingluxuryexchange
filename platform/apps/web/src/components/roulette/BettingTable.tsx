...
export function BettingTable({ chip, bets, disabled, onPlaceBet }: Props) {
  ...
  // Reset split selection when a new betting round opens
  useEffect(() => {
    ...
    <div className="space-y-1">
      ...
      <div className="flex gap-1 overflow-x-auto no-scrollbar">
        {(Object.keys(MODE_LABELS) as BetMode[]).map(m => (
          <button
            key={m}
            onClick={() => { if (!disabled) { setBetMode(m); setSplitFirst(null); } }}
            disabled={disabled}
            className={`shrink-0 px-2.5 py-1 rounded-md text-[9px] uppercase tracking-wider font-bold transition-all border ${
              betMode === m
                ? "bg-white/20 text-white border-white/30"
                : "text-white/35 border-white/10 hover:text-white/60 hover:border-white/20"
            }`}
            style={{ background: betMode === m ? "rgba(255,255,255,0.12)" : "transparent" }}
          >
            {MODE_LABELS[m]}
          </button>
        ))}
      ...
      {/* Mode hint (only for non-straight modes) */}
      {betMode !== "straight" && (
        <div className="text-center text-[9px] text-yellow-300 uppercase tracking-widest font-semibold leading-tight">
          {betMode === "split"   && splitFirst === null && "↓ Click 1st number, then click an adjacent number"}
          {betMode === "split"   && splitFirst !== null && `↓ ${splitFirst} selected — click an adjacent number`}
          {betMode === "street"  && "↓ Click any number to bet all 3 in that row"}
          {betMode === "sixline" && "↓ Click any number to bet 6 numbers (2 rows)"}
          {betMode === "corner"  && "↓ Click top-left of a 2×2 block to bet the corner"}
        </div>
      )}
