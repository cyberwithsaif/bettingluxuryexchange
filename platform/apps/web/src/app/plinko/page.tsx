...
export default function PlinkoPage() {
  ...
  // Refs for auto-play loop
  const autoRef      = useRef(false);
  const animDoneRef  = useRef<(() => void) | null>(null);

  // ── Fetch config & multiplier table ──────────────────────────────────────
  ...
  const drop = useCallback(async (): Promise<boolean> => {
    if (!token) { toast.error("Please log in to play"); return false; }
    if (!config?.enabled) { toast.error("Plinko is currently disabled"); return false; }
    if (animating && !turbo) return false;
    if (betAmount < (config?.minBet ?? 10)) {
      toast.error(`Minimum bet is ₹${config?.minBet}`); return false;
    }
    setDropping(true);
    try {
      ...
      if (!res.ok) { toast.error(data.message ?? "Bet failed"); return false; }
      ...
      if (data.multiplier >= 10) {
        toast.success(`🎉 ${data.multiplier}× — Won ₹${data.payout.toLocaleString()}!`, { duration: 4000 });
      }
      return true;
    } catch {
  ...
  }, [token, config, animating, turbo, betAmount, rows, risk, clientSeed, fetchBalance]);
  ...
  const startAuto = useCallback(async () => {
    ...
    while (left > 0 && autoRef.current) {
      ...
      if (stopOnProfit > 0 && profit >= stopOnProfit) { toast.info("Auto-play: profit target reached"); break; }
      if (stopOnLoss   > 0 && profit <= -stopOnLoss)  { toast.info("Auto-play: loss limit reached"); break; }
    }
    autoRef.current = false;
    setAutoPlay(false);
  }, [autoCount, animating, drop, sessionProfit, stopOnProfit, stopOnLoss]);
  ...
  const double = () => setBetAmount(a => Math.min(config?.maxBet ?? 100000, a * 2));
    ...
    <div className="min-h-screen bg-[#0d0e15] text-white flex flex-col">
      ...
      <div className="flex flex-1 flex-col lg:flex-row gap-0 overflow-hidden">
        ...
        <div className="w-full lg:w-72 shrink-0 bg-[#13141f] border-r border-white/8 flex flex-col p-4 gap-4 overflow-y-auto">
          ...
          <div>
            ...
            <div className="flex gap-2 mt-1.5">
              ...
              <button onClick={double} className="flex-1 text-xs py-1 rounded bg-white/8 hover:bg-white/15 transition">2×</button>
              <button onClick={() => setBetAmount(balance ?? 100)}
                className="flex-1 text-xs py-1 rounded bg-white/8 hover:bg-white/15 transition">Max</button>
            </div>
          </div>
              ...
              {ROWS_OPTIONS.map(r => (
                <button key={r} onClick={() => setRows(r)}
                  className={`py-2 rounded-lg text-xs font-bold transition ${
                    rows === r ? "bg-purple-600 text-white" : "bg-white/8 text-white/60 hover:bg-white/15"
                  }`}>
                  {r}
                </button>
              ))}
          ...
          <div className="flex gap-3">
            <label className="flex items-center gap-2 text-xs cursor-pointer select-none">
              ...
              <Zap size={14} className="text-yellow-400" /> Turbo
            </label>
          </div>
          ...
          <button
            ...
            className="w-full py-3.5 rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 font-bold text-lg tracking-wide
              disabled:opacity-40 hover:brightness-110 transition shadow-lg shadow-purple-900/40"
          >
          ...
          <details className="group">
            <summary className="text-xs uppercase tracking-wider text-white/50 cursor-pointer flex items-center gap-1 select-none">
            ...
            <div className="mt-3 space-y-3">
              <div>
                ...
                <input type="number" min={1} max={1000} value={autoCount}
              ...
              {autoPlay && (
                <div className="text-xs text-white/60">
                  ...
                  P/L: <span className={sessionProfit >= 0 ? "text-green-400" : "text-red-400"}>
                    ₹{sessionProfit.toFixed(0)}
                  </span>
                </div>
              )}
              <button onClick={handleAuto} disabled={!user}
                className={`w-full py-2.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition disabled:opacity-40 ${
                  autoPlay ? "bg-red-700 hover:bg-red-600" : "bg-white/10 hover:bg-white/20"
                }`}>
              ...
              <div>
                ...
                <div className="flex gap-1">
                  ...
                  <button onClick={() => setClientSeed(Math.random().toString(36).slice(2, 12))}
                    className="p-1 rounded bg-white/8 hover:bg-white/15">
                    <RotateCcw size={12} />
                  </button>
                </div>
              </div>
              <p className="text-xs text-white/40">
                Verify any bet at <span className="text-purple-400">/api/plinko/verify/:betId</span>
              </p>
            </div>
          </details>
        </div>
              ...
              {result && !animating && result.multiplier >= 2 && (
                ...
                >
                  <div className={`px-6 py-3 rounded-2xl backdrop-blur-md text-center shadow-2xl border border-white/20
                    ${result.multiplier >= 100 ? "bg-white/20" :
                      result.multiplier >= 20  ? "bg-yellow-900/60" :
                      result.multiplier >= 5   ? "bg-orange-900/60" : "bg-green-900/60"}`}>
          ...
          {history.length > 0 && (
            <div className="border-t border-white/8 px-4 py-2 flex gap-2 overflow-x-auto no-scrollbar">
              {history.slice(0, 30).map((h, i) => (
                <div key={i} className={`shrink-0 text-xs px-2 py-1 rounded font-bold ${multColor(h.multiplier)}`}>
                  {h.multiplier}×
                </div>
              ))}
            </div>
        ...
        <div className="w-full lg:w-64 shrink-0 bg-[#13141f] border-l border-white/8 flex flex-col">
          <div className="px-4 py-3 border-b border-white/8 text-xs uppercase tracking-wider text-white/50">
            Live Bets
          </div>
          <div className="flex-1 overflow-y-auto">
            ...
            {liveFeed.length === 0 && (
              <div className="p-4 text-xs text-white/30 text-center">Waiting for bets…</div>
            )}
          </div>
        </div>
      </div>
