            ...
            {(data ?? []).map((t: any) => (
              <tr key={t.id} className="border-t border-gray-100 hover:bg-yellow-50/30 transition">
                ...
                <Td className="flex gap-1">
                  {t.status === "PENDING" && (
                    <>
                      <button onClick={() => approve(t.id)} className="text-xs px-2 py-1 rounded-lg border border-emerald-200 text-emerald-700 bg-emerald-50 hover:bg-emerald-100 font-medium transition">Approve</button>
                      <button onClick={() => reject(t.id)} className="text-xs px-2 py-1 rounded-lg border border-red-200 text-red-600 bg-red-50 hover:bg-red-100 font-medium transition">Reject</button>
                    </>
                  )}
                </Td>
              </tr>
