  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-gray-400 mt-0.5">Admin actions and system events</p>
      </div>
            ...
            {!isLoading && (!data || (data as any[]).length === 0) && (
              <tr>
                <td colSpan={6} className="text-center py-12 text-gray-400">
                  ...
                  <p className="text-xs mt-1 text-gray-300">Admin actions will appear here.</p>
                </td>
              </tr>
            )}
            {(data as any[] ?? []).map((l: any) => (
              <tr key={l.id} className="border-t border-gray-100 hover:bg-yellow-50/30 transition align-top">
                ...
                <Td className="font-semibold text-gray-800">
                  {l.actor?.username ?? "—"}
                  <span className="text-xs text-gray-400 ml-1">{l.actor?.role}</span>
                </Td>
