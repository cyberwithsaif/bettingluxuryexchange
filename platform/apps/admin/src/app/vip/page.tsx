...
import { Crown, Plus, Trash2, UserPlus, Percent, Gift, Award } from "lucide-react";
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <PageHeader title="VIP Management" subtitle="Loyalty tiers, cashback & rewards"
        right={<button onClick={() => setCreating((v) => !v)} className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold text-gray-900 bg-gradient-to-r from-yellow-400 to-yellow-500 hover:brightness-110 transition"><Plus size={16} /> New Tier</button>}
      />
      ...
      {creating && (
        <GlassCard className="p-5 animate-slide-in-down">
          <h2 className="font-black text-gray-100 mb-4">New VIP Tier</h2>
          ...
          <div className="flex gap-2 mt-4">
            <button onClick={createLevel} disabled={busy} className="px-4 py-2 rounded-lg text-sm font-bold text-gray-900 bg-gradient-to-r from-yellow-400 to-yellow-500 hover:brightness-110 disabled:opacity-50 transition">{busy ? "Saving…" : "Create Tier"}</button>
          ...
          {data!.map((l) => (
            <GlassCard key={l.id} glow className="p-5 relative overflow-hidden">
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => removeLevel(l.id, l.name)} className="p-1.5 rounded-lg text-gray-500 hover:text-red-400 hover:bg-red-500/10 transition"><Trash2 size={15} /></button>
              </div>
